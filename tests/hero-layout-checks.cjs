...
assert.match(css, /\.hero-sequence-pending \.hero-unified \.hero-main-title[\s\S]*opacity: 0;[\s\S]*filter: blur\(\.18rem\);/, 'Headline and hero details wait invisibly in their final layout');
assert.match(css, /\.hero-sequence-ready \.hero-unified \.hero-main-title \{ transition-delay: 0ms; \}[\s\S]*\.hero-sequence-ready \.hero-server-rack \{ transition-delay: 90ms; \}[\s\S]*\.hero-sequence-ready \.hero-system-screen \.monitor-services \{ transition-delay: 150ms; \}[\s\S]*\.hero-sequence-ready \.hero-unified \.hero-copy \.hero-actions \{ transition-delay: 230ms; \}/, 'The overlapping reveal uses one restrained order');
...
assert.match(css, /\.hero-unified \.hero-copy \{[^}]*grid-template-columns: minmax\(0, 1fr\);/s, 'The unified hero resets the legacy split layout to one full-width column');
assert.match(css, /\.hero-server-stage \{[^}]*grid-column: 1;[^}]*grid-row: 1;/s, 'The server stage spans the full unified hero width');
assert.match(css, /\.hero-unified \.hero-copy \.hero-actions \{[^}]*grid-column: 1;[^}]*grid-row: 2;[^}]*justify-self: center;[^}]*margin: clamp\(\.4rem, 1\.2svh, 1rem\) auto 0;/s, 'Hero actions stay centered below the full-width stage');
assert.match(css, /\.hero-terminal-shell\.hero-unified\s*\{[^}]*grid-template-rows: minmax\(0, 42%\) minmax\(0, 58%\);[^}]*height: calc\(100svh - 6\.25rem\);[^}]*overflow: hidden;/s, 'The complete desktop hero owns exactly the available viewport');
assert.match(css, /@media \(max-width: 760px\)[\s\S]*\.hero-terminal-shell\.hero-unified \{[^}]*height: calc\(100svh - var\(--mobile-site-header-height\)\);/s, 'The complete mobile hero owns the viewport below navigation');
