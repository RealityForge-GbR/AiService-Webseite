...
assert.match(viewportRefinements, /@media \(min-width: 1101px\) \{[\s\S]*#unsere-sicht \.view-people-layout:not\(\.is-view-copy-stacked\) \.view-copy \{[\s\S]*display: grid;[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[\s\S]*columns: auto;/s, 'Desktop team copy uses deliberate paragraph pairs instead of auto-flowing newspaper columns');
assert.match(viewportRefinements, /@media \(max-width: 760px\) \{[\s\S]*#unsere-sicht \.view-copy \{[\s\S]*columns: 1;/s, 'The approved phone team composition remains a single-column disclosure');
...
async function checkInteractions(width, reducedMotion) {
  ...
  if (width <= 760 && !reducedMotion) {
    ...
    assert.equal(lifted.style.getPropertyValue('--strategy-lift-cable-height'), '139.0px', 'The mobile cable reaches the crane boom above the stacked block');
